const SlideShow = () => {
  const slideShow = document.querySelector(".slide_show_genre");
  const prevButton = slideShow.querySelector(".prev");
  const nextButton = slideShow.querySelector(".next");
  const slides = slideShow.querySelector(".slides_container_genre");
  let currentSlide = 0;
  slides.children[0].classList.add("selected_slide_genre");
  ...
  const setCurrentSlide = (slide_number) => {
    ...
    slides.children[currentSlide].classList.remove("selected_slide_genre");
    navigation.children[currentSlide].classList.remove("dotted");
    currentSlide = slide_number;
    slides.children[currentSlide].classList.add("selected_slide_genre");
