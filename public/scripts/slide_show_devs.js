const SlideShow = () => {
  ...
  const slideShow = document.querySelector(".slide_show_developer");
  const prevButton = slideShow.querySelector(".prev");
  const nextButton = slideShow.querySelector(".next");
  const slides = slideShow.querySelector(".slides_container_developer");
  slides.children[0].classList.add("selected_slide_developer");
  ...
  const setCurrentSlide = (slide_number) => {
    ...
    slides.children[currentSlide].classList.remove("selected_slide_developer");
    navigation.children[currentSlide].classList.remove("dotted");
    currentSlide = slide_number;
    slides.children[currentSlide].classList.add("selected_slide_developer");
